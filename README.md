...
Every one of them exists because something broke first. The failure that bought each tool is written at the top of its README, including the times my first fix was wrong.
...
- [path-fence](https://github.com/justin-rhee/path-fence). Refuses reads of credential-shaped paths, after resolving the path properly first.
...
- [alert-redactor](https://github.com/justin-rhee/alert-redactor). Builds alerts carrying the shape of a failure and none of its content.
...
Each one is small, has offline tests, and is honest about what it can't do. If a check can be fooled, I say how in its README.

## What building these taught me

The tools are the easy part to show. The reasoning behind them is in
[LESSONS.md](LESSONS.md): what I got wrong, what it cost, and the rules I now
follow because of it. The shortest version is that a check reporting success
without having run is worse than having no check at all, and I have learned that
